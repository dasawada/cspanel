...
export function createCannedMessagesPanel(options = {}) {
  ...
  async function doIdentifyCourseWithSignal(inputVal, signal, seq) {
    ...
    if (teacherLeave && isFirstCourse) {
      ...
      // 首課專用格式
      apiTexts.tab4 = `行政\t\t${taipeiMMDD}\t請假與補課\t老師假單\t${studentNames}\t${mmddTime} 首課老師請假，未安排代課\thttps://oneclub.backstage.oneclass.com.tw/audition/course/edit/${courseId}\tTRUE\tTRUE`;
    ...
    我們已安排代課老師，感謝您的理解與支持！`;
      ...
      apiTexts.tab4 = `行政\t\t${taipeiMMDD}\t請假與補課\t老師假單\t${studentNames}\t${mmddTime} 老師請假，已排代課\thttps://oneclub.backstage.oneclass.com.tw/audition/course/edit/${courseId}\tTRUE\tTRUE`;
      panel.querySelector(`#${panelId}-tab4 textarea`).value = apiTexts.tab4;
