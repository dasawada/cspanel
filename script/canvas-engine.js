    ...
    const detach = makeDraggable(el, handle, {
      ...
      onPositionChange: (pos) => {
        ...
        if (target && commitGroup(target, p.id)) {
          ...
          // 計時器可能在此之後把 el.style.left/top 覆寫回落點座標，蓋掉
          // commitGroup 剛寫入的頁內定位。無論本次 drop 是否真的觸發過回彈都
          // 補一次重新斷言——非回彈路徑下這只是重跑一次等效的 layout()，
          // 冪等、無副作用（不影響「成組後拖曳面板不寫畫布 layout」——本呼叫
          // 只重申既有頁內定位，不寫 saveLayoutEntry／畫布 layout）。
          setTimeout(() => {
            const wm = window.WindowManager;
            if (wm && typeof wm.syncPanes === 'function') wm.syncPanes();
